...
describe('pushButton tests', () => {
  ...
  describe('theming', () => {
    it('allows theming the `min-width` property', () => {
      const component = baseComponent({customStyle: { minWidth: '300px' } })
      const tree = create(component).toJSON()
      expect(tree).toHaveStyleRule('min-width', '300px')
    })

    it('allows theming the `min-height` property', () => {
      const component = baseComponent({customStyle: { minHeight: '50px' } })
      const tree = create(component).toJSON()
      expect(tree).toHaveStyleRule('min-height', '50px')
    })

    it('allows theming the `font-size` property', () => {
      const component = baseComponent({customStyle: { fontSize: '40px' } })
      const tree = create(component).toJSON()

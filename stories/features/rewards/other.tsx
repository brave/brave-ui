...
storiesOf('Feature Components/Rewards/Other/Desktop', module)
  ...
  .add(
    'Box',
    withState({ checked: false, toggle: true, settings: false }, store => {
      const onToggle = () => {
      ...
      return (
        <div style={{ width: '595px' }}>
          <Box
            ...
            type={select(
              'Type',
              { contribute: 'contribute', donation: 'donation', ads: 'ads' },
              'contribute'
            )}
            description={text(
              'Description',
              `Earn tokens by seeing ads on Brave. Ads are matched
  ...
  .add('Disabled content', () => {
    return (
      ...
      >
        • Donate on the spot as you find gems. <br />• <b>Enable Tips </b> on
        Twitter, YouTube, and more, to give tips to posts you ‘Like’.
      </DisabledContent>
    )
  })
  .add('Alert', () => {
  ...
  .add(
    'Donate',
    withState({ donationAmounts, currentAmount: '5.0' }, store => {
      const onDonate = () => {
  ...
  .add('List', () => {
    return (
      <div style={{ width: '595px' }}>
        <List title={text('Title', 'Earnings this month')}>Some content</List>
      </div>
    )
  })
  .add('List - Token', () => {
    ...
    return (
      <div style={{ width: '400px' }}>
        <Profile
          ...
          src={bart}
          showUnVerifiedHelpIcon={boolean('Show unverified icon', false)}
        />
      </div>
    )
  })
  ...
  .add('Grant claim', () => {
    return <GrantClaim onClaim={dummyClick} />
  })
  .add('Panel Welcome', () => {
    return (
  ...
  .add(
    'Toggle Tips',
    withState({ tipsEnabled: true }, store => {
      const onToggle = () => {
    ...
    return (
      <div>
        <div style={{ position: 'absolute', top: '200px', left: '700px' }}>
          <Tooltip id={'tooltip-test'} content={braveAdsText}>
        ...
        <div style={{ position: 'absolute', top: '280px', left: '685px' }}>
          <Tooltip id={'tooltip-test'} content={batLogoText}>
        ...
        <div style={{ position: 'absolute', top: '390px', left: '670px' }}>
          <Tooltip id={'tooltip-test'} content={styledUpholdContent}>
  ...
  .add(
    ...
    withState({ displayed: true }, store => {
      ...
      return (
        <div
          style={{
            background: `url(${tipScreen}) no-repeat top center`,
            width: '986px',
            height: '100vh',
            margin: '0 auto',
            position: 'relative'
  ...
  .add(
    'Tab',
    withState({ tabIndexSelected: 0 }, store => {
      const onSwitch = () => {
...
storiesOf('Feature Components/Rewards/Other/Mobile', module)
  ...
  .add(
    'Box',
    withState({ checked: true, toggle: true }, store => {
      const onToggle = () => {
        store.set({ checked: !store.state.checked })
      }
      ...
      return (
        <div style={{ width: '100%' }}>
          <BoxMobile
            ...
            description={text(
              'Description',
              `A simple way to support content creators. Set monthly allowance and browse normally. Your favorite sites (content sites only) receive your contributions automatically. You can exclude sites from funding right from the list below.`
            )}
